    ...
    def get(self):
        ...
        def test_getasin():
            urls = ['https://www.amazon.com/Nintendo-Switch-Gray-Joy-Con/dp/B01LTHP2ZK?tag=kasbl023-20',
                ...
                'https://www.amazon.com/gp/product/B003ZK58SQ/ref=s9_acsd_ri_bw_c_x_4_w?pf_rd_m=ATVPDKIKX0DER&pf_rd_s=merchandised-search-8&pf_rd_r=RC30RJCRNJHMPQK8KEH5&pf_rd_r=RC30RJCRNJHMPQK8KEH5&pf_rd_t=101&pf_rd_p=b8c0a303-a08e-4b0b-bd49-040811fd7080&pf_rd_p=b8c0a303-a08e-4b0b-bd49-040811fd7080&pf_rd_i=283155'
            ]
            for url in urls:
                getasin(url)
        ...
        print("Precio_envio: %s, Cotizacion: %s" % (precio_envio, cotizacion))
        ...
        if(itemasin):
            ...
            template_values = {
                    'error': "No se encontro un numero ASIN en la URL",
                    'error_summary': "Favor, ingrese una URL completa con el ASIN!",
                }
            template = JINJA_ENVIRONMENT.get_template('index.html')
            self.response.write(template.render(template_values))
